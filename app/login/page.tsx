...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Usa o método oficial do NextAuth para Client Components
      const result = await signIn('credentials', {
        email,
        password,
        redirect: false, // Evita que a página recarregue bruscamente
      })

      if (result?.error) {
        setError('E-mail ou senha incorretos. Tente novamente.')
      } else {
        // Sucesso! Redireciona e atualiza o estado global
        router.push('/')
        router.refresh()
      }
    } catch (err) {
      ...
      <div className="w-full max-w-md space-y-8 rounded-2xl bg-white p-8 shadow-xl border border-gray-100">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            Entre para gerenciar seus pedidos e compras
          </p>
        </div>
